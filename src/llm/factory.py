...
# V1 - Added azure_foundry provider entry (Azure AI Foundry endpoint)
#
...
#   - Real providers (openai, azure_openai, azure_foundry, anthropic) are imported
#     lazily inside create() so that missing provider files do not break imports
#     in tests.
...
#   2. Add one entry to the if-block inside create()
...
class LLMProviderFactory:
    ...
    def create(settings: Settings) -> LLMProvider:
        ...
        if provider == "azure_foundry":
            from src.llm.azure_foundry_provider import AzureFoundryProvider  # noqa: PLC0415
        ...
        raise UnknownProviderError(
            ...
            f"Valid values are: mock, openai, azure_openai, azure_foundry, anthropic. "
