...
# V1 - Fixed PromptExample: removed duplicate model_config and inner Config class.
#      Pydantic v2 does not allow both model_config and class Config together.
#      populate_by_name=True kept in single model_config declaration.
#
...
class PromptExample(BaseModel):
    ...
    schema_:   compressed table/column summary shown to the LLM in the example.
               YAML key is 'schema' — remapped to 'schema_' by _remap_example_schema_keys()
               in settings.py before Pydantic sees it, to avoid collision with
