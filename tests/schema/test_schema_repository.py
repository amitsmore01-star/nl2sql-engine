...
def _minimal_schema(app_id: str = "ABC_app") -> dict:
...
class TestSchemaRepository:
    ...
    def test_loads_valid_abc_schema(self, tmp_path):
        """Load valid ABC_app.json — returns AppSchema with correct appId."""
        _write_schema(tmp_path, "ABC_app.json", _minimal_schema("ABC_app"))
        repo = SchemaRepository()
        repo.load(tmp_path)
        schemas = repo.get_all_schemas()
        assert len(schemas) == 1
        assert schemas[0].appId == "ABC_app"

    # --- Scenario 2 ---
    def test_get_schema_returns_correct_schema(self, tmp_path):
        """get_schema('ABC_app') returns AppSchema with all expected tables."""
        _write_schema(tmp_path, "ABC_app.json", _minimal_schema("ABC_app"))
        repo = SchemaRepository()
        repo.load(tmp_path)
        schema = repo.get_schema("ABC_app")
        assert isinstance(schema, AppSchema)
        assert schema.appId == "ABC_app"
    ...
    def test_get_all_schemas_returns_list(self, tmp_path):
        ...
        assert all_schemas[0].appId == "ABC_app"
    ...
    def test_load_raises_if_filename_does_not_match_app_id(self, tmp_path):
        """File named wrong_name.json but appId inside is ABC_app — raises SchemaLoadError."""
        _write_schema(tmp_path, "wrong_name.json", _minimal_schema("ABC_app"))
        repo = SchemaRepository()
        with pytest.raises(SchemaLoadError) as exc_info:
            repo.load(tmp_path)
        assert exc_info.value.code == "SCHEMA_LOAD_ERROR"
    ...
    def test_load_succeeds_when_filename_matches_app_id(self, tmp_path):
        """File named ABC_app.json with appId ABC_app inside — loads successfully."""
        _write_schema(tmp_path, "ABC_app.json", _minimal_schema("ABC_app"))
        repo = SchemaRepository()
        repo.load(tmp_path)  # Must not raise
        assert repo.get_schema("ABC_app").appId == "ABC_app"
